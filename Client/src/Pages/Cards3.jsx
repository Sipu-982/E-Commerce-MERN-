...
const Cards3 = () => {
  ...
  const fetchPopularElectronic = async () => {
    setLoading(true);
    try {
      const res = await axios.get(
        'http://localhost:3004/api/product/filterProduct?category=Electronic'
      );
  ...
  useEffect(() => {
    fetchPopularElectronic();
  }, []);

  return (
    <div className="bg-white p-4 w-[490px] min-h-[550px] max-w-2xl mx-auto">
      {/* Header */}
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold">Best Selling Cameras</h2>
